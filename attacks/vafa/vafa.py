...
class VAFA(Attack):
    ...
    def __init__(self, model,loss_fn, batch_size=1, channels=1, height=96, width=96, depth=96, steps=20, q_min=5, q_max=10,  block_size=(8,8,8),
                 vafa_norm=True, use_ssim_loss=False, targeted=False, verbose=True):
    ...
    def forward(self, images, labels):
        ...
        for i in range(self.steps):
            ...
            if success_rate >= 1:
                ...
                merged_q_tables = block_merging_3d(self.q_tables.detach(),
                                                   images.shape)  # [B, C, N_Blocks, Block_H, Block_W, Block_D] --> [B,C,H,W,D]

                return adv_images, pred_labels, merged_q_tables


        adv_images = torch.clamp(adv_images, min=0, max=1.0).detach()
        merged_q_tables = block_merging_3d(self.q_tables.detach(), images.shape) # [B, C, N_Blocks, Block_H, Block_W, Block_D] --> [B,C,H,W,D]
