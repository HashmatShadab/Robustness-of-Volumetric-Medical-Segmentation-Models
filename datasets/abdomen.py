...
def get_loader_abdomen(args):
    ...
    train_transform = transforms.Compose(
        [
            ...
            #transforms.Spacingd(keys=["image", "label"], pixdim=(args.space_x, args.space_y, args.space_z), mode=("bilinear", "nearest")), #No need to do the resampling as I have resampled it already offline
            ...
            transforms.SpatialPadd(keys=["image", "label"], spatial_size=[-1, -1, 245], mode='constant', method='end'), #The maximum depth is 245 during training and as we take a batch size >1, hence to be consistent.
    ...
    val_transform = transforms.Compose(
        [
            ...
            transforms.CropForegroundd(keys=["image", "label"], source_key="image"),
            transforms.SpatialPadd(keys=["image", "label"], spatial_size=[-1, -1, 96], mode='constant', method='end'), #here keep 96 because we need a minimum depth of 96 at the time of inference
            transforms.ToTensord(keys=["image", "label"]),
        ]
    )
